        ...
        if i.custom_id in buttons:
            ...
            if res or i.member.permissions & hikari.Permissions.ADMINISTRATOR:
                ...
                embed.add_field(f"Updated: {i.custom_id}",
                                ...
                                inline=True)
                await i.create_initial_response(response_type=hikari.ResponseType.MESSAGE_CREATE,embed=embed,flags=hikari.MessageFlag.EPHEMERAL)
...
@link.child()
@lightbulb.check_exempt(utils.mod_check)
@lightbulb.option("blocker","What you want to block",choices=["social_media","google","bitly","discord","GIF","all_links"],required=True)
